...
@section('titulo','Listado de incidentes registrado')
@section('contenido')

	<h2 class="text-center"><span class="glyphicon glyphicon-list"></span> Listado de Incidentes registrados</h2>

    <a href="{{route('incidente.generar.pdp')}}" class="btn btn-danger"><i class="fa fa-file-pdf-o fa-lg" aria-hidden="true"></i></a>
	<hr> 
    ...
    <script>
        ...
        $(document).ready(function() {
            var table = $('#tabla_tecnico').DataTable( {
                //"ajax": "http://localhost/Laravel/SistemaIncidentes/public/incidente/tabla-tecnico-propio-ajax",
                "ajax": "{{url('incidente/tabla-tecnico-propio-ajax')}}",
                "destroy":true,
                "columns": [
                    { "data": "id" },
                    { "data": "tipo_incidente" },
                ...
                "createdRow" : function(row,data,index){
                    ...
                    if(data['estado'] == 'abierto'){
                        $('td:eq(3)',row).html('<span class="label label-success">Abierto</span>');
                    }else if(data['estado'] == 'cerrado'){
                        $('td:eq(3)',row).html('<span class="label label-danger">Cerrado</span>');
                    }else{
                        $('td:eq(3)',row).html('-');
                    }
                    if(data['id'] == 0){
